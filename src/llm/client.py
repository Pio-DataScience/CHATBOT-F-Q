...
class LMClient:
    def __init__(
        ...
        logger.info(
            "Retry delay: %ds (backoff factor: %.1fx)",
            self.retry_delay,
            self.backoff_factor,
        )
    ...
    def _is_valid_response(self, content):
        ...
        if (
            any(indicator in lower_content for indicator in error_indicators)
            and len(stripped) < 50
        ):
    ...
    def _make_request(self, url, payload, attempt):
        ...
        logger.info(
            "[Attempt %d/%d] Sending request to LLM API", attempt, self.max_retries
        )
        ...
        try:
            ...
            if finish_reason == "length" and not content:
                logger.error(
                    "Model hit token limit during reasoning phase - content field is empty"
                )
                logger.error(
                    "This means max_tokens is too small. Need to increase it significantly."
                )
                raise LLMEmptyResponseError(
                    "Response truncated during reasoning - increase max_tokens"
                )
            ...
            if not self._is_valid_response(content):
                ...
                raise LLMEmptyResponseError(
                    f"Invalid or empty content: '{content[:100]}'"
                )
            ...
            logger.info(
                "✓ Received valid response (%d characters) in %.2f seconds",
                len(content),
                elapsed,
            )
            logger.debug(
            ...
            logger.error(
                "✗ HTTP error from LLM API: %s (status: %d)",
                e,
    ...
    def chat(self, messages, max_tokens=1024):
        ...
        for i, msg in enumerate(messages):
            ...
            logger.debug(
                "Message %d [%s]: %s", i + 1, msg.get("role", "?"), content_preview
            )
        ...
        for attempt in range(1, self.max_retries + 1):
            try:
                ...
                if attempt < self.max_retries:
                    ...
                    logger.warning(
                        "Attempt %d/%d failed: %s", attempt, self.max_retries, str(e)
                    )
